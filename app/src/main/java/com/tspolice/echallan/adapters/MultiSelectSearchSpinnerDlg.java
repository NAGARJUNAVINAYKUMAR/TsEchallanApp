...
import com.tspolice.echallan.utils.RecyclerViewEmptySupport;


import java.util.ArrayList;


public class MultiSelectSearchSpinnerDlg extends AppCompatDialogFragment implements SearchView.OnQueryTextListener, View.OnClickListener {
    ...
    public MultiSelectSearchSpinnerDlg title(String title) {
        this.title = title;
        return this;
    }

    public MultiSelectSearchSpinnerDlg titleSize(float titleSize) {
        this.titleSize = titleSize;
        return this;
    }

    public MultiSelectSearchSpinnerDlg positiveText(@NonNull String message) {
        this.positiveText = message;
        return this;
    }

    public MultiSelectSearchSpinnerDlg negativeText(@NonNull String message) {
        this.negativeText = message;
        return this;
    }

    public MultiSelectSearchSpinnerDlg preSelectIDsList(ArrayList<Integer> list) {
        this.previouslySelectedIdsList = list;
        this.tempPreviouslySelectedIdsList = new ArrayList<>(previouslySelectedIdsList);
        return this;
    }

    public MultiSelectSearchSpinnerDlg multiSelectList(ArrayList<MultiSelectModel> list) {
    ...
    public MultiSelectSearchSpinnerDlg setMaxSelectionLimit(int limit){
        this.maxSelectionLimit = limit;
        return this;
    }

    public MultiSelectSearchSpinnerDlg setMaxSelectionMessage(String message) {
        this.maxSelectionMessage = message;
        return this;
    }

    public MultiSelectSearchSpinnerDlg setMinSelectionLimit(int limit){
        this.minSelectionLimit = limit;
        return this;
    }

    public MultiSelectSearchSpinnerDlg setMinSelectionMessage(String message) {
        this.minSelectionMessage = message;
        return this;
    }

    public MultiSelectSearchSpinnerDlg onSubmit(@NonNull SubmitCallbackListener callback) {
    ...
    private ArrayList<MultiSelectModel> filter(ArrayList<MultiSelectModel> models, String query) {
        ...
        for (MultiSelectModel model : models) {
            final String name = model.getVltnSecName().toLowerCase();
    ...
    private String getSelectedDataString() {
        ...
        for (int i = 0; i < tempMainListOfAdapter.size(); i++) {
            if (checkForSelection(tempMainListOfAdapter.get(i).getId())) {
                data = data + ", " + tempMainListOfAdapter.get(i).getVltnSecName();
            }
        }
        if (data.length() > 0) {
    ...
    private ArrayList<String> getSelectNameList() {
        ...
        for(int i=0;i<tempMainListOfAdapter.size();i++){
            if(checkForSelection(tempMainListOfAdapter.get(i).getId())){
                names.add(tempMainListOfAdapter.get(i).getVltnSecName());
            }
        }
        return names;
    }
